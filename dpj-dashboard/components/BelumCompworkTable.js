...
// ── Inline editor untuk TANGGAL SETTING ─────────────────────────────────────
function TglSettingCell({ row, onSaved }) {
  ...
  // Format date input ke DD/MM/YYYY untuk dikirim ke Apps Script
  ...
  // Format date ISO ke YYYY-MM-DD untuk input type=date
  const toInputDate = (displayStr) => {
    if (!displayStr || displayStr === "-") return "";
    // coba parse "26 Jun 2026" atau "01/05/2026"
  ...
  const handleEdit = () => {
    setValue(toInputDate(row.tanggalSetting));
    setSaved(false);
    setEditing(true);
  };
  ...
  const handleSave = useCallback(async () => {
    ...
    setError("");
    try {
      const tglForSheet = formatForSheet(value);
      const res = await fetch("/api/update-setting", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ workorderPsb: row.workorderPsb, tanggalSetting: tglForSheet }),
      });
      ...
      const displayVal = `${d.getDate()} ${BULAN[d.getMonth()]} ${d.getFullYear()}`;
      if (onSaved) onSaved(row.workorderPsb, displayVal);
      ...
      <td className="tglCell editing">
        <div className="editWrap">
          ...
          <div className="editActions">
            ...
            <button type="button" onClick={handleCancel} disabled={saving} className="btnCancel" title="Batal (Esc)">
              ✕
            </button>
          </div>
        </div>
...
export default function BelumCompworkTable({ rows, compworkValues }) {
  ...
  const [localTgl, setLocalTgl] = useState({});

  const handleTglSaved = useCallback((workorderPsb, newTgl) => {
  ...
  const mergedRows = useMemo(() =>
    belumCompwork.map((r) =>
      r.workorderPsb in localTgl
        ? { ...r, tanggalSetting: localTgl[r.workorderPsb] }
        : r
    ),
    [belumCompwork, localTgl]
  );
  ...
  return (
    <div className="wrap">
      ...
      <div className="editHint">
        <span>✏ Klik ikon pensil pada kolom <strong>Tgl Setting</strong> untuk update jadwal — order akan otomatis pindah ke tabel "Order Setting Hari Ini" saat tanggal sesuai</span>
      </div>
          ...
          <thead>
            <tr>
              ...
              {COLUMNS.map((c) => (
                <th key={c.key}>
                  {c.label}
                  {c.key === "tanggalSetting" && <span className="editableHint"> ✏</span>}
                </th>
              ))}
            </tr>
          </thead>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={COLUMNS.length + 1} className="emptyCell">
                  {belumCompwork.length === 0
                    ? "🎉 Semua order sudah COMPWORK!"
      ...
      <style jsx>{`
        ...
        .editableHint { color: var(--warning, #f59e0b); font-size: 10px; }
        tbody td {
